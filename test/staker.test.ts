...
describe("Staker", async function () {
  it("Should Set Rewards", async function () {
    ...
    const tx = await staker.setRewards(ethers.utils.parseUnits(String(TOTAL_REWARDS)), DURATION);

    const blockTimestamp = (await ethers.provider.getBlock(tx.blockHash as string)).timestamp;

    expect(await staker.duration()).to.eq(DURATION);
    expect(await staker.rewardsSupply()).to.eq(ethers.utils.parseUnits(String(TOTAL_REWARDS)));
    expect(await staker.rewardRate()).to.eq(ethers.utils.parseUnits(String(TOTAL_REWARDS)).div(DURATION));
    expect((await staker.lastUpdateTime()).toNumber()).to.eq(blockTimestamp);
    expect((await staker.finishAt()).toNumber()).to.eq(blockTimestamp + DURATION);
  });
  it("Should Not Set Rewards (insufficient funds)", async function () {
    ...
    await expect(staker.setRewards(ethers.utils.parseUnits(String(TOTAL_REWARDS)), DURATION)).reverted;
  });
  ...
  //     await new Promise((res, rej) => setTimeout(res, DURATION * 1000));

  //     await staker.connect(otherAccount).withdraw(ethers.utils.parseEther("1"));
  //     await staker.connect(otherAccount).getRewards();

  //     console.log(Number(ethers.utils.formatEther(await celestia.balanceOf(otherAccount.address))));
  //   });
});
